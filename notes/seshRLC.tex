...
v.Comment on the current magnification
Based on part (iv), we see that while the main source current entering the circuit from the supply is only $2\text{ mA}$, the internal circulating reactive currents flowing through the inductor and capacitor branches are $100\text{ mA}$.
This confirms a clear amplification effect where the branch reactive currents are exactly $50\text{ times}$ larger than the supply line current, which matches our calculated quality factor ($Q = 50$). In a parallel resonant circuit, the inductor and capacitor exchange energy locally, keeping the source from having to supply it.
